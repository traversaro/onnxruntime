...
from ._sympy_utils import parse_shape, sympy_symbol
...
class CodegenContext:
    ...
    def get_internal_variable_name(self, name: str):
...
class NodeVisitor:
    @abstractmethod
    def codegen(self, node: Any, context: CodegenContext, indent: int):
        pass
...
class TensorInfo:
    def __init__(self, dtype, shape: List):
        self._dtype = dtype
        self._shape = parse_shape(shape)
...
def _infer_elementwise_shape(input_infos: List[TensorInfo]):
...
def _infer_elementwise(node: NodeProto, input_infos: List[TensorInfo]):
    return TensorInfo(input_infos[0].dtype, _infer_elementwise_shape(input_infos))


def _infer_where(node: NodeProto, input_infos: List[TensorInfo]):
    return TensorInfo(input_infos[1].dtype, _infer_elementwise_shape(input_infos))


def _infer_reduction(node: NodeProto, input_infos: List[TensorInfo]):
    # Support reduction on the last axis only for now.
    return TensorInfo(input_infos[0].dtype, input_infos[0].shape[:-1] + [sympy_symbol(1)])


def _infer_unary(node: NodeProto, input_infos: List[TensorInfo]):
    return input_infos[0]


def _infer_cast(node: NodeProto, input_infos: List[TensorInfo]):
...
def _infer_dropout(node: NodeProto, input_infos: List[TensorInfo]):
...
class TypeAndShapeInfer(object):
    ...
    @classmethod
    def infer(cls, node: NodeProto, input_infos: List[TensorInfo]):
